...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const addDownloadGroup = (name = '', links = []) => {
    ...
    groupContainer.innerHTML = `
      ...
      <div class="bulk-url-container">
        ...
        <div class="bulk-url-options">
          <div class="label-prefix-container">
            <label for="label-prefix-${groupId}">Label Prefix:</label>
            <input type="text" id="label-prefix-${groupId}" value="PART" placeholder="Label Prefix">
          </div>
          <div class="process-options">
            <label>
              <input type="checkbox" id="append-urls-${groupId}" checked>
        ...
        <div class="bulk-actions">
          ...
          <button type="button" class="clear-all-btn" data-group-id="${groupId}">Clear All Links</button>
        </div>
      </div>
    ...
    // Add event listener for clearing all links
    groupContainer.querySelector('.clear-all-btn').addEventListener('click', function() {
      const groupId = this.getAttribute('data-group-id');
      clearAllLinks(groupId);
    });
  ...
  // Clear all links in a group
  function clearAllLinks(groupId) {
    const linksContainer = document.getElementById(`group-${groupId}-links`);
    
    if (!linksContainer) return;
    
    if (!confirm('Are you sure you want to remove all links in this group?')) {
      return;
    }
    
    // Keep only one empty link
    ...
    linksContainer.querySelector('input[name="link-url[]"]').addEventListener('paste', handleUrlPaste);
  }
  ...
  function processBulkUrls(groupId) {
    ...
    const linksContainer = document.getElementById(`group-${groupId}-links`);
    const labelPrefix = document.getElementById(`label-prefix-${groupId}`).value || 'PART';
    const appendUrls = document.getElementById(`append-urls-${groupId}`).checked;
    ...
    // If not appending, clear existing links except the first one
    if (!appendUrls) {
      ...
      labelInput.value = `${labelPrefix}1`;
      urlInput.value = urls[0];
      
      // Start from the second URL
      for (let i = 1; i < urls.length; i++) {
        addLinkWithValues(groupId, `${labelPrefix}${i+1}`, urls[i]);
      }
    } else {
      // Append mode - find the next index number
      const existingLabels = Array.from(linksContainer.querySelectorAll('input[name="link-label[]"]'))
        .map(input => input.value);
      
      let nextIndex = 1;
      
      // Try to find the highest existing index for this prefix
      const regex = new RegExp(`^${labelPrefix}(\\d+)$`);
      ...
      // Add all URLs with incrementing indices
      for (let i = 0; i < urls.length; i++) {
        addLinkWithValues(groupId, `${labelPrefix}${nextIndex + i}`, urls[i]);
      }
    }
  ...
  // Add a link with specific values
  function addLinkWithValues(groupId, label, url) {
    const linksContainer = document.getElementById(`group-${groupId}-links`);
    if (!linksContainer) return;
    ...
    row.innerHTML = `
      <input type="text" name="link-label[]" placeholder="Label" value="${label}" required>
      <input type="url" name="link-url[]" placeholder="URL" value="${url}" required>
      <button type="button" class="remove-link-btn">Remove</button>
    `;
  ...
  function handleUrlPaste(e) {
    ...
    if (pastedText.includes('\n')) {
      ...
      // Ask user if they want to process all URLs
      if (confirm(`${urls.length} URLs detected. Do you want to process all of them?`)) {
